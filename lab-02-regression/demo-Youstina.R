...
## to tabulate education by female
with(cps, table(occupation,fe))
xtabs(~ occupation + fe, data=cps)
summary(xtabs(~occupation + fe, data=cps))

xtabs(~ occupation + fe + year, data=cps)

prop.table(xtabs(~ occupation + fe, data=cps))
prop.table(xtabs(~ occupation + fe, data=cps), margin=2)
prop.table(xtabs(~ occupation + fe, data=cps), margin=1)

## to create a new variable but it won't change the dataset
mutate(cps,
       wage= exp(lnwage))

## to create a new variable in the dataset, be careful with this method because it changes your dataset
cps<- mutate(cps,
             wage=exp(lnwage))
...
##graphing , aes refers to aestatic, geom is the form of the graph (points, lines, or bars)
...
## Heteroskedasticity-consistent (robust) standard errors, lm is the function for regression and it stands for linear model
