...
library(Hmisc) ## For some nicely formatted summary stats (careful summarize conflict)
...
cpi <- fredr::fredr_series_observations("CPIAUCNS", observation_start = as.Date("1978-01-01"),
             observation_end = as.Date("2022-12-31"),
             frequency = "a", aggregation_method = "avg")
cpi <- cpi %>% transmute(year = year(date), cpi = value) %>% filter(year %in% c(1978, 1985, 2022))
...
cps %>% group_by(year) %>% summarize(mean(lnwage), mean(exp(lnwage)), exp(mean(lnwage)), sd(exp(lnwage)), sd(lnwage))

cps$lnwage - mean(cps$lnwage)
...
cps %>% tabyl(occupation, fe) %>% adorn_percentages("row")

cps %>% tabyl(occupation, fe) %>% adorn_percentages("col")
...
cps %>% as_factor() %>% tabyl(occupation, fe) %>% adorn_percentages("row") %>% adorn_title()

cps %>% as_factor() %>% tabyl(occupation, fe) %>% adorn_percentages("col") %>% adorn_title()

cps %>% as_factor() %>% tabyl(occupation, industry) %>% adorn_percentages("row") %>% adorn_title()
cps %>% as_factor() %>% tabyl(occupation, industry) %>% adorn_percentages("col") %>% adorn_title()

cps %>% as_factor() %>% tabyl(occupation, industry, fe) %>% adorn_title()

cps %>% as_factor() %>% tabyl(occupation, industry, fe) %>% adorn_percentages() %>% adorn_title("combined")
...
with(cps, tapply(exp(lnwage), list(year, as_factor(ethnic)), median))

with(cps, tapply(exp(lnwage), list(year, as_factor(ethnic)), sd))

with(cps, tapply(lnwage, list(year, as_factor(ethnic)), sd))


cps %>% as_factor() %>% group_by(year, ethnic) %>% summarize(sd(lnwage))


## Join the CPI data to compute real wages
...
summarize(group_by(cps, year), mean(lnwage), sd(lnwage), mean(exp(lnwage)), sd(exp(lnwage)), exp(mean(lnwage)))
summarize(group_by(cps, year), mean(lnrwage2022), sd(lnrwage2022), mean(exp(lnrwage2022)), sd(exp(lnrwage2022)), exp(mean(lnrwage2022)))
